...
class QuizPopup(Popup):
        ...
        self.questions = [
            {
                ...
                'options': ['Luftaufnahmen machen', 'Drohnenrennen gewinnen', 'Sicherheit und Gesetzeskonformität gewährleisten'],
                'correct_answer': 'Sicherheit und Gesetzeskonformität gewährleisten'
            },
            {
                'question': 'Welche Flugregeln sollten beim Betrieb einer Drohne beachtet werden?',
                'options': ['Es gibt keine Regeln', 'Je höher, desto besser', 'Luftraumvorschriften und Datenschutzbestimmungen beachten'],
                'correct_answer': 'Luftraumvorschriften und Datenschutzbestimmungen beachten'
            },
            {
                'question': 'Warum ist die Kenntnis der Drohnentechnik wichtig?',
                'options': ['Nur für Technik-Enthusiasten', 'Zum Angeben vor Freunden', 'Sicherer Betrieb und Steuerung der Drohne'],
                'correct_answer': 'Sicherer Betrieb und Steuerung der Drohne'
            }
        ]

        self.question_label = Label(text=self.questions[self.current_question_index]['question'], font_size=20)
        self.option_buttons = []
    ...
    def show_quiz_result(self):
        result_popup = Popup(title="Quiz Ergebnis", content=Label(text=f"Du hast {self.score} von {len(self.questions)} Fragen richtig beantwortet."),
...
class MyButtonsApp(App):
    ...
    def show_popup(self, instance, btn_name):
        if btn_name == "Vorbereitung Drohnenführerschein":
            ...
            if content is not None:
                popup = Popup(title=title, content=content, size_hint=(None, None), size=(400, 800))
                close_button = Button(text="zurück", on_press=lambda *args: self.dismiss_popup(popup),
                                      size_hint=(None, None), size=(80, 50), pos_hint={'top': 2, 'right': 0})
                content.add_widget(close_button)
                popup.open()

    def create_table_content(self):
        table_layout = GridLayout(cols=3, spacing=12, size_hint_y=None, pos_hint={'top': 0.95})

        header_label1 = Label(text="Felder", bold=True, font_size=18)
        header_label2 = Label(text="Zustand", bold=True, font_size=18)
        header_label3 = Label(text="zuletzt bewässert", bold=True, font_size=18)
        ...
        for feld, zustand_text, zuletzt_bewaessert_text in zip(felder, zustand, zuletzt_bewaessert):
            label_feld = Label(text=feld, font_size=12, size_hint_y=None, height=30)
            label_zustand = Label(text=zustand_text, font_size=12, size_hint_y=None, height=30)
            label_zuletzt_bewaessert = Label(text=zuletzt_bewaessert_text, font_size=12, size_hint_y=None, height=30)
    ...
    def create_hydrojet_info(self):
        hydrojet_info_layout = GridLayout(cols=1, spacing=12, size_hint_y=None, pos_hint={'top':0.75})
        ...
        label_hydrojet_info = Label(text=hydrojet_info_text, font_size=16, size_hint_y=None, height=55)
    ...
    def create_drone_info(self):
        ...
        header_label1 = Label(text="Infos", bold=True, font_size=18)
        header_label2 = Label(text="verbundene Drohne", bold=True, font_size=18)
        ...
        for label_text, value_text in drone_data:
            label = Label(text=label_text, font_size=12, size_hint_y=None, height=30)
            value = Label(text=value_text, font_size=12, size_hint_y=None, height=30)
...
class PreparationSubPopup(Popup, EventDispatcher):
    def __init__(self, **kwargs):
        ...
        close_button = Button(text="zurück", on_press=lambda *args: self.dismiss(), size_hint=(None, None),
                              size=(50, 30), pos_hint={'top': 1, 'right': 0.065})
    ...
    def show_pruefungsanmeldung_popup(self):
        pruefungsanmeldung_text = ("Für die Anmeldung: Besuchen Sie uns gerne auf unserer Website.")
        ...
        close_button = Button(text="zurück", on_press=pruefungsanmeldung_popup.dismiss,
                              size_hint=(None, None), size=(50, 30), pos_hint={'top': 1, 'right': 1})
...
def open_course_info_popup():
    course_info_text = """
    1.  Gesetzliche Compliance sicherstellen: Informieren Sie sich über die aktuellen nationalen und 
         regionalen Gesetze und Vorschriften für den Betrieb von Drohnen. Unsere Schulung deckt alle
         relevanten rechtlichen Anforderungen ab.

    2.  Teilnahme an theoretischer Schulung: Besuchen Sie unsere theoretische Schulung, die alle
         notwendigen Kenntnisse abdeckt, darunter Flugregeln, Luftraumklassen, Datenschutzbestimmungen
         und aktuelle gesetzliche Änderungen.

    3.  Absolvierung der praktischen Schulung: Nehmen Sie an unserer praxisorientierten Schulung teil,
         die praktische Flugübungen, Navigationsszenarien und reale Anwendungsfälle umfasst. Hier lernen
         Sie, die Drohne sicher zu steuern und auf verschiedene Situationen zu reagieren.

    4. Kenntnis der Drohnentechnik: Verstehen Sie die Technik Ihrer Drohne. Unsere Schulung deckt
         grundlegende technische Aspekte ab, von der Steuerung bis zur GPS-Navigation.

    5.  Luftfahrtkarteninterpretation: Lernen Sie, Luftfahrtkarten zu lesen und zu verstehen, um sicherzustellen,
         dass Sie während des Flugs innerhalb erlaubter Lufträume agieren.

    6.  Notfallprozeduren beherrschen: Unsere Schulung umfasst umfassende Notfallprozeduren, damit Sie in 
         kritischen Situationen angemessen und sicher reagieren können.

    7.  Versicherungsnachweis: Stellen Sie sicher, dass Sie die erforderlichen Versicherungen haben. Unsere
         Schulung hilft Ihnen dabei,die richtige Deckung zu verstehen und sicherzustellen.

    9.  Praxisorientierte Anwendung: Setzen Sie Ihr Wissen in der Praxis um, indem Sie realistische Flugszenarien
         durchlaufen. Unsere Schulung fördert die praktische Anwendung von Theorie.
